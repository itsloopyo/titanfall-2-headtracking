...
:: Titanfall 2 imports dsound.dll; the Ultimate ASI Loader renamed to
:: dsound.dll proxies it and loads our .asi from the same directory.
:: Titanfall2.exe is at the game root, so the .asi + loader land at the
:: game root. HeadTracking.ini + the log are created next to Titanfall2.exe
:: by the mod on first launch, so neither is deployed here.
set "ASI_LOADER_NAME=dsound.dll"
set "MOD_CONTROLS=Controls: End=toggle tracking, PgUp=cycle 6DOF/rotation/position, PgDn=toggle yaw mode, Insert=cycle ADS mode. Chords: Ctrl+Shift+Y/G/H/U."
